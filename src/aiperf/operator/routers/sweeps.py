...
def _read_search_summary(aggregate_path: str | None) -> SweepSearchSummary | None:
    ...
    directory in the namespace (``job_union._scan_pvc_jobs``).
...
async def _cells_from_live_children(
    ...
    A cell is emitted for every child carrying a ``variation_index``, terminal
    or not; only terminal children contribute to the trial counts and metric
    averages. Returns an empty list when no child carries a ``variation_index``.
    """
...
# aggregation completion. The children route falls back to enumerating the
# labelled child CRs and synthesizing the same response shape.
