...
"""Tests for GPU benchmark with different endpoint types against real SGLang."""
...
class TestGPUEndpointTypes:
    """Tests for different endpoint types against a real SGLang server."""
    ...
    async def test_endpoint_type_completes_successfully(
        ...
        logger.info(
            f"[TEST] Endpoint type test: type={endpoint_type}, model={sglang_config.model_name}, url={sglang_endpoint_url}"
        )
