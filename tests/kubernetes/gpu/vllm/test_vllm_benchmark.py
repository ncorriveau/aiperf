...
"""Tests for GPU benchmark execution, lifecycle, and concurrency scaling."""
...
class TestGPUBenchmarkCompletion:
    """Tests for GPU benchmark completion (module-scoped for speed)."""
...
class TestGPUBenchmarkLifecycle:
    """Tests for GPU benchmark lifecycle management (function-scoped)."""
...
class TestGPUBenchmarkConcurrency:
    """Tests for GPU benchmark with different concurrency levels."""
    ...
    async def test_benchmark_succeeds_at_concurrency_level(
        ...
        logger.info(
            f"[TEST] Concurrency test: level={concurrency}, model={vllm_config.model_name}"
        )
